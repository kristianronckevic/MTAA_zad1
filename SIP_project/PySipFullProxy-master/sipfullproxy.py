...
class UDPHandler(socketserver.BaseRequestHandler):
    ...
    def processRegister(self):
        ...
        self.debugRegister()
        self.sendResponse("200 0K (zmena)")
    ...
    def processInvite(self):
        ...
        if len(origin) == 0 or not origin in registrar:
            self.sendResponse("400 Bad Request (nejde)")
            #
            actual = datetime.now()
            date = actual.strftime("%d/%m/%Y %H:%M:%S")

            file = open("dennik.txt", "a")  # append mode
            file.write(date + "\r\n")
            file.write("**********Hovor ukončený**********\n")
            file.write("**********************************\n")
            file.close()
            #
            return
        ...
        if len(destination) > 0:
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                file = open("dennik.txt", "a")  # append mode
                file.write("*********************************\n")
                file.write("*******Inicializujem hovor*******\n")
                file.write(date + "\n Volaný: " + destination + "\n Volajúci: " + origin + "\r\n")
                file.close()
                #
            else:
                self.sendResponse("480 Temporarily Unavailable (nedostupny)")
                #
                actual = datetime.now()
                date = actual.strftime("%d/%m/%Y %H:%M:%S")
